...
namespace Lab1
{
    public class Blue
    {
        ...
        public bool Task8(int a, int b, int c)
        {
            ...
            // Проверяем, что во всех мешках хотя бы по 1 (не считая мелкого) монете
            if (hA >= 1 && hB >= 1 && hC >= 0)
            {
                // Уже норм?
                if (hA == hB && hB == hC)
                ...
                // Если не норм то смотрим с учетом алиментов на младшего
                    ...
                    answer = true;
            //else if (hA == 0 || hB == 0 || hC == 0)  уже прописано что тру будет только если есть хотя бы 1 монета у каждого
                //    if (hA == hB && hB == hC)
                //        answer = true;
            }
            // end
...
//у меня был выбор: изучать всё это с 0 и писать лабу либо быть счастливой и играть в сапера..... Выбор был сделан не в пользу счастья Т-Т